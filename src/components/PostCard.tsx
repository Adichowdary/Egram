...
export function PostCard({ post: initialPost, user, getInitials, onDelete }: PostCardProps) {
    ...
    return (
        ...
        >
                ...
                <div className="flex items-center gap-3.5 min-w-0">
                    ...
                    <div className="flex flex-col min-w-0">
                        <div className="flex items-center gap-1.5 min-w-0">
                            <span className="text-sm sm:text-base font-extrabold tracking-tight truncate" style={{ color: "var(--text-dark)" }}>{post.authorName}</span>
                            <CheckCircle2 className="w-4 h-4 text-blue-500 fill-blue-500/20 flex-shrink-0" />
                        </div>
                        <div className="flex items-center gap-2 text-xs font-semibold" style={{ color: "var(--text-light)" }}>
                            <span className="px-2 py-0.2 rounded-full font-bold border" style={{ backgroundColor: "var(--primary-bg)", color: "var(--primary)", borderColor: "var(--card-border)" }}>Student</span>
                            <span>•</span>
                    ...
                    <AnimatePresence>
                        {showMenu && (
                            <motion.div
                                ...
                                className="absolute right-0 mt-2 w-48 border rounded-xl shadow-2xl z-50 overflow-hidden p-1.5 space-y-1"
                                style={{ backgroundColor: "var(--accent-bg)", borderColor: "var(--card-border)" }}
                            >
                                <button
                                    onClick={handleShare}
                                    className="w-full text-left px-3.5 py-2 text-xs font-bold rounded-lg transition-all flex items-center gap-2"
                                ...
                                {post.authorId === user.uid && (
                                    <button
                                        ...
                                        className="w-full text-left px-3.5 py-2 text-xs font-bold text-rose-500 hover:bg-rose-500/10 rounded-lg transition-all flex items-center gap-2"
                                    >
            ...
            <div className="flex items-center justify-between p-4 sm:p-5 border-t" style={{ borderColor: "var(--card-border)" }}>
                <div className="flex items-center gap-5">
                    <motion.button
                        whileTap={{ scale: 0.75 }}
                        onClick={handleLike}
                        className={`flex items-center gap-1.5 transition-colors cursor-pointer text-xs sm:text-sm font-bold ${isLiked ? 'text-rose-500' : ''}`}
                ...
                <button 
                    ...
                    className={`transition-colors cursor-pointer ${isSaved ? 'text-blue-500' : ''}`}
            ...
            {/* 4. Text Content Caption Widget - Auto Height Expansion */}
            <div className="px-4 pb-4 sm:px-5 sm:pb-5 space-y-2">
                <p className="text-xs sm:text-sm font-medium leading-relaxed break-words" style={{ color: "var(--text-dark)" }}>
                    {post.content}
                </p>
                ...
                {post.commentsCount && post.commentsCount > 0 && !showComments && (
                    <button
                        ...
                        className="text-xs font-bold transition-colors pt-1 block cursor-pointer"
                        style={{ color: "var(--text-light)" }}
                    >
                ...
                {showComments && (
                    ...
                    >
                        <div className="max-h-64 overflow-y-auto p-4 sm:p-5 space-y-3 no-scrollbar">
                            {isLoadingComments ? (
                                <div className="py-4 text-center text-xs flex items-center justify-center gap-2 font-bold" style={{ color: "var(--text-light)" }}>
                                    <div className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" />
                                    <span>Loading comments...</span>
                                </div>
                            ) : comments.length === 0 ? (
                                <p className="text-xs font-medium text-center py-2" style={{ color: "var(--text-light)" }}>No comments yet. Start the conversation!</p>
                            ) : (
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="leading-snug break-words flex flex-wrap items-baseline gap-x-1.5">
                                                    ...
                                                    <span className="font-normal break-words" style={{ color: "var(--text-light)" }}>{comment.text}</span>
                                                </div>
                                                <div className="flex items-center gap-3 mt-1 text-[11px] font-medium" style={{ color: "var(--text-light)" }}>
                                            ...
                                            <button 
                                                onClick={() => handleToggleCommentLike(comment.id)}
                                                className={`p-1 transition-colors cursor-pointer ${isCommentLiked ? 'text-rose-500' : ''}`}
                        ...
                        <form onSubmit={handleAddComment} className="flex items-center gap-2 px-4 py-3 border-t" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--card-border)" }}>
                            <input
                                type="text"
                                value={newComment}
                            ...
                            <button
                                ...
                                className="text-blue-500 font-bold text-xs sm:text-sm disabled:opacity-40 px-3 py-1 transition-all cursor-pointer flex items-center gap-1 rounded-lg"
